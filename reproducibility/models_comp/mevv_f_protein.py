...
Usage:
    python mevv_f.py --data_flag protein_inter --flag fitness
"""
...
parser.add_argument('--flag', type=str, default='fitness', help="Task type: 'fitness', 'epistasis', 'subtle', or 'strong'")
parser.add_argument('--data_flag', type=str, default='protein_inter', help="Dataset name: 'harry', 'elife', or 'protein_inter'")
...
if config.data_flag == 'harry':
    ...
    if flag == 'fitness':
        data_path = '../../data/protein_inter/protein_mevv_fitness.csv'
    else:
        data_path = '../../data/protein_inter/protein_mevv_epi.csv'
...
# ===== Load Data =====
data_df = pd.read_csv(data_path, sep=sep)
...
val_frac[-1] = 1.0
val_id = choice(range(num_samples), int(num_samples * 0.3), replace=False)
train_full_id = [i for i in range(num_samples) if i not in val_id]
...
def run_one(i: int, frac: float) -> float:
    ...
    cor_m = pearsonr(fit_mave, data_df['y'][val_id])[0]
    return cor_m
